...
class AntiSpam(Plugin):
    ...
    def check_message_violations(self, player, name, message):
        ...
        if time.monotonic() - last_time < self.config["message_delay"]:
            self.warn_player(player, f"{ColorFormat.RED} Bạn gửi tin nhắn quá nhanh!")
            return True

        if self.blocked_words_regex.search(message.lower()):
            self.warn_player(player, f"{ColorFormat.RED} Tin nhắn của bạn có chứa từ cấm!")
            return True
    ...
    def warn_player(self, player, reason: str) -> None:
        ...
        player.send_message(f"{ColorFormat.YELLOW}Cảnh báo {warns}/{max_warns}: {reason}")

        if warns >= max_warns:
            kick_message = self.config.get("kick_message", "You have been kicked for receiving too many warnings!")
            player.kick(kick_message)
            self.logger.info(f"{player.name} bị kick vì nhận quá nhiều cảnh báo.")
